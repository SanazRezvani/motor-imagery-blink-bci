...
def main():
    ...
    print("Extracting combined features (CSP + bandpower + variance)...")
    ...
    # 2. Bandpower + variance features
    X_bp = extract_epoch_features(
        epochs,
        motor_channels=DECODING_CHANNELS,
        sfreq=SFREQ,
    )

    # 3. Combine all features
    import numpy as np
    X = np.concatenate([X_csp, X_bp], axis=1)
